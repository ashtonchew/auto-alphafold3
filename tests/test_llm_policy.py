...
from autoalphafold3.llm_policy import AgentSearchPhase, LLMPhasePolicy, default_llm_phase_policy
...
def test_patch_planning_policy_disables_web_search_and_uses_medium_reasoning() -> None:
...
def test_phase_policy_rejects_contract_drift() -> None:
    ...
    with pytest.raises(ValidationError, match="patch planning uses medium reasoning"):
        LLMPhasePolicy(
...
def test_agent_llm_policy_cli_outputs_agents_sdk_spec(capsys: pytest.CaptureFixture[str]) -> None:
    ...
    assert payload["model_settings"]["reasoning"] == {"effort": "medium"}
